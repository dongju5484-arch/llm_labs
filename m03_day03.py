#===============================================================
# 멀티모달 ai 튜터 -> ai_doc_tutur/app.py
# - PDF 업로드
# - 라마인덱스로 RAG 인덱스 생성
# - 질문/답변
# - 근거 페이지 표시
...
APP_TITLE = 'ai문서 / 강의자료 튜터'
DEFAULT_MODEL = 'gpt-4o-mini'
DEFAULT_EMBED_MODEL = 'text-embedding-3-small'

@st.cache_resource(show_spinner=False)
def configure_llama_index() -> None:
    """라마인덱스전역 설정을 한번만 구성하는 함수"""
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        raise RuntimeError('openai api key가 없습니다. .env를 확인하세요')
...
def build_index_from_pdf(file_bytes: bytes, filename: str) -> VectorStoreIndex:
    """업로드된 PDF 바이트를 임시 파일로 저장한 뒤 RAG 인덱스를 생성하는 함수"""
    configure_llama_index() # 설정 함수 호출 

    with tempfile.TemporaryDirectory() as temp_dif:
        pdf_path = Path(temp_dif) / filename
        pdf_path.write_bytes(file_bytes)

        documents = SimpleDirectoryReader(input_files=[str(pdf_path)]).load_data()

        if not documents:
            raise ValueError('PDF에서 텍스트를 읽지 못했습니다.')
        
        index = VectorStoreIndex.from_documents(documents)
        return index
    
def format_source_info(response) -> list[dict]:
    """라마인덱스 응답에서 근거 정보를 화면 표시용으로 정리하는 함수"""
    results = []
...
def main() -> None:
    ...
    st.caption('PDF 강의자료를 업로드하고 문서 기반 질문을 해보는 RAG 입문 프로젝트!')

    with st.sidebar:
        st.header('수업용 설정')
        st.write(f'LLM: {DEFAULT_MODEL}')
        st.write(f'Embedding: {DEFAULT_EMBED_MODEL}')
        st.info('스캔본 PDF는 텍스트 추출이 안될 수 있습니다! 텍스트 PDF를 권장합니다!')

    uploaded_file = st.file_uploader('PDF 파일 업로드', type=['pdf'])

    if uploaded_file is None:
        st.warning('먼저 PDF를 업로드하세요. 샘플은 data/sample_lecture.pdf를 사용하세요.')
        return
    
    file_bytes = uploaded_file.getvalue()

    try:
        with st.spinner('문서를 읽고 인덱스를 만드는 중입니다. 시간이 조금 걸릴 수도 있습니다.'):
            index = build_index_from_pdf(file_bytes, uploaded_file.name) # 함수 호출
    ...
    with col1:
        question = st.text_area(
            '질문 입력',
            value='이 문서의 핵심 내용은 초보자도 이해할 수 있게 요약해줘.',
            height=100,
        )
        ask = st.button('질문하기', type='primary')

    with col2:
        top_k = st.sidebar('검색할 근거 수', min_value=1, max_value=5, value=2)
